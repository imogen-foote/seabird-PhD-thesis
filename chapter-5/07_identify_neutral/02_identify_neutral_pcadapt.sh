...
bgzip -ci $TMP/$SET'_tmp.recode.vcf' > $TMP/$SET'_tmp.vcf.gz'

#convert tmp to gds
Rscript $vcf2R --gzvcf $TMP/$SET'_tmp.vcf.gz' 	\
               --snprelate_out $TMP/$SET'_tmp'
#convert tmp to plink
Rscript $gds2plink --gds_file $TMP/$SET'_tmp.gds' \
                   --out $TMP/$SET'_tmp'          \
                   --pop_file $pop_file

#filter linked sites
plink --bfile $TMP/$SET'_tmp'		\
	--indep-pairwise 50 5 0.2	\
	--chr-set 65			\
	--out $TMP/$SET'_tmp'
sed -i 's/:/\t/g' $TMP/$SET'_tmp.prune.in' 	#replace colons with tabs to make file compatible with vcftools
...
vcftools --gzvcf $TMP/$SET'_tmp.vcf.gz'		\
		--out $VCF'_neutral'					\
		--positions $TMP/$SET'_tmp.prune.in'	\
		--recode-INFO-all --recode
#uses output file from previous step of SNPs to keep

#convert to vcf.gz
#mv $VCF'_neutral.recode.vcf' $VCF'_neutral.vcf'
bgzip -ci $VCF'_neutral.recode.vcf' > $VCF'_neutral.vcf.gz'
